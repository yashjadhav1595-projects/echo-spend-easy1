...
app.post('/api/drive/backup', async (req, res) => {
  try {
    ...
    if (!accessToken || !data) {
      return res.status(400).json({ error: 'Missing access token or data' });
    }
    ...
    const metadata = {
      ...
      description: 'Echo Spend Easy - Financial Data Backup',
    };
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: `Upload failed: ${errorText}` });
    }

    const result = await response.json();
    res.json({ success: true, fileName: result.name, fileId: result.id });
...
app.get('/api/drive/list', async (req, res) => {
  try {
    ...
    const response = await fetch(
      'https://www.googleapis.com/drive/v3/files?q=name%20contains%20%27echo-spend-easy-backup%27%20and%20trashed%20%3D%20false&orderBy=createdTime%20desc&fields=files(id,name,createdTime,size)',
      {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
        },
    ...
    if (!response.ok) {
      return res.status(response.status).json({ error: 'Failed to list files' });
    }
